...
def MaskTrain(helper, start_epoch, local_model, target_model, is_poison,agent_name_keys):
    agent_name_keys = [10]
    epochs_submit_update_dict = dict()
    num_samples_dict = dict()
    current_number_of_adversaries=0
    ...
    for model_id in range(helper.params['no_models']):
        ...
        for epoch in range(start_epoch, start_epoch + helper.params['aggr_epoch_interval']):
            ...
            if is_poison and agent_name_key in helper.params['adversary_list'] and (epoch in localmodel_poison_epochs):
                ...
                from optimize_mask_cifar import mask_train, get_mask_scores, save_mask_scores
                from prune_neuron_cifar import prune_by_threshold, read_data
                _, data_iterator = helper.train_data[agent_name_key]
                local_model = replace_bn_with_noisy_bn(model)
                local_model = local_model.to(device)
                local_model.mask_lr = 0.2
                local_model.anp_eps = 0.4
                local_model.anp_steps = 1
                local_model.anp_alpha = 0.6
                mask_scores = None
                device = 'cuda:0'
                ...
                for epoch in range(5):
                    train_loss, train_acc = mask_train(model=local_model, criterion=criterion, data_loader=data_iterator,
                                                mask_opt=mask_optimizer, noise_opt=noise_optimizer, helper = helper)
                    print(f'epoch:{epoch} train_loss:{train_loss} train_acc:{train_acc}')
                mask_scores = get_mask_scores(local_model.state_dict())
                save_mask_scores(local_model.state_dict(), f'save/mask_values.txt')
                mask_values = read_data(f'save/mask_values.txt')
                mask_values = sorted(mask_values, key=lambda x: float(x[2]))
                # prune_by_threshold(local_model, mask_values, pruning_max=0.4, pruning_step=0.5)
                num_samples_dict[agent_name_key] = 0
